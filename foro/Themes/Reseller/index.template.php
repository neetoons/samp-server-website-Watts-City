			...
			if ($context['user']['is_logged'])
			{
				...
				if ( $facData[0] == 1 )
				{
					...
					if ( $num <= 0 )
					{
						...
						$q = $bridge->query("UPDATE ".$db_prefix."members SET usertitle = '<b>Lider</b> de <b>".$facData2['Nombre']."</b>' WHERE member_name = '".$username."'");
				...
				else
				{
					...
					if ( $num >= 1 )
					{
						$q = $bridge->query("DELETE FROM ".$db_prefix."moderators WHERE id_member = ".$fData['id_member']." AND id_board = ".$id['id_board']);
					}
				}

				if ( $facData[1] == "0" )
				{
					$q = $bridge->query("UPDATE ".$db_prefix."members SET usertitle = '<b>Usuario</b>' WHERE member_name = '".$username."'");
				...
				{
					$numFacs = $bridge->query("SELECT id, Rango1, Rango2, Rango3, Rango4, Rango5, Rango6, Rango7, Rango8, Nombre FROM facciones WHERE id = ".$facData[1]);
					$ranks = $numFacs->fetch_array(MYSQLI_ASSOC);

					$q = $bridge->query("UPDATE ".$db_prefix."members SET usertitle = '<b>".$ranks['Rango'.$facData[2]]."</b> de <b>".$ranks['Nombre']."</b>' WHERE member_name = '".$username."'");
		...
		<footer>
			<div class="container">
				<div class="row">
					...
					<div class="col-lg-12">
						Watts City 2013 - 2015 by Josh Hawes
					</div>
					<div class="col-lg-12">
